...
<template>
    <div v-if="!isInAdminProductsRoute && !isEntregasRoute" st class="layout-topbar shadow-3 px-3 md:pl-5 md:pr-5 lg:pl-7 lg:pr-7 "
        style=" z-index:999;background: linear-gradient(to right, #ffcf00, red);">
        ...
        <button @click="(siteStore.setVisible('currentSite',true))" class="p-link boton-menu layout-topbar-logo text-white"
            style=" font-size: 24px;  color: black; z-index: 99999;display:flex; justify-content: center; align-items: center;">

            <i sty :class="PrimeIcons.MAP_MARKER" style="font-size: 100%;font-weight: bo; color: var(--primary-color)"> </i>
        ...
        <button v-if="isSmallScreen" style="border: none" class="p-link layout-menu-button layout-topbar-button"
            @click="onMenuToggle()">
            <i style="border: ; color: var(--primary-color);" class="text-4xl pi pi-bars text-white"></i>
        </button>
        ...
        <div class="layout-topbar-menu " :class="topbarMenuClasses" v-if="!isInAdminProductsRoute">
            ...
            <div class=" botones-menu" :class="topbarMenuClasses" v-for=" menuTopbar in menuOptions"
                ...
                <router-link :to="`/${menuTopbar.to}`" @click="handleDropDownMenu(false)" class="">
                    <Button  text size="small"  :label="menuTopbar.name" class="p-link boton-menu text-xl py-3 px-3 text-white" style="font-weight: bold; box-shadow: none; color: var(--text-color);"
        ...
        <div class="layout-topbar-menu" :class="topbarMenuClasses" style="z-index: 999 ;">
            ...
            <router-link to="/cart">
                <button class="p-link layout-topbar-button p-0" style="position: relative;">
                    <i badge="5+" :class="PrimeIcons.SHOPPING_CART" class="text-white"></i>
                    <div class="p-0"
...
.selected::after {
      ...
      width: 100%;
      height: 0;
      border-bottom: .3rem solid white; /* Borde inferior */
    }
